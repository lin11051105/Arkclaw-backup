...
- For this skill, prefer using skill-scoped variables such as `MODEL_IMAGE_API_KEY` and `MODEL_IMAGE_API_BASE` whenever possible.
...
```bash
python3 {baseDir}/scripts/seedream_image_generate.py -p "A cute kitten playing in a garden"
```
...
```bash
# Use 4.0 version
python3 {baseDir}/scripts/seedream_image_generate.py -p "A cute kitten" --version 4.0

# Use 4.5 version
python3 {baseDir}/scripts/seedream_image_generate.py -p "A cute kitten" --version 4.5

# Use 5.0-lite version (recommended)
python3 {baseDir}/scripts/seedream_image_generate.py -p "A cute kitten" --version 5.0
```
...
python3 {baseDir}/scripts/seedream_image_generate.py -p "Beautiful sunset" -s 2048x2048 --no-watermark --version 5.0

# Batch generation
python3 {baseDir}/scripts/seedream_image_generate.py -p "Generate 3 cute dog pictures" -g --max-images 3 --version 4.5

# Image-to-image
python3 {baseDir}/scripts/seedream_image_generate.py -p "Convert this image to anime style" -i "https://example.com/image.jpg" --version 5.0

# Image-to-image with local file
python3 {baseDir}/scripts/seedream_image_generate.py -p "Convert this image to anime style" -i ./photo.jpg --version 5.0

# Web search tool (5.0-lite only)
python3 {baseDir}/scripts/seedream_image_generate.py -p "Latest 2026 smartphone" --web-search --version 5.0

# Custom output format (5.0-lite only)
python3 {baseDir}/scripts/seedream_image_generate.py -p "A beautiful landscape" --output-format png --version 5.0

# List all supported versions
python3 {baseDir}/scripts/seedream_image_generate.py --list-versions
```
...
| `--image` | `-i` | Single reference image URL or local file path | - |
| `--images` | - | Multiple reference image URLs or local paths (space separated) | - |
...
A: Common formats like JPG and PNG are supported, provided via URL, local file path, or Base64. Local files are automatically converted to base64 data URIs. Max file size: 50MB (warning at 20MB). System paths and sensitive directories are blocked for security.
