...
Do not directly modify the system-level global environment variables `ARK_API_KEY` and `ARK_BASE_URL` unless you fully understand the global impact.

- Modifying these global variables can break image generation and video generation skills by causing authentication failures or request routing issues.
- It can also break the global API routing behavior of the OpenClaw gateway, which may cause request failures or broader service instability.
- For this skill, prefer using skill-scoped variables such as `MODEL_VIDEO_API_KEY` and `MODEL_VIDEO_API_BASE` whenever possible.
...
- `first_frame` (str): URL or local file path for the first frame image
- `last_frame` (str): URL or local file path for the last frame image
- `reference_images` (list[str]): 1-4 reference image URLs or local file paths for style/content guidance
- `reference_videos` (list[str]): 0-3 reference video URLs or local file paths (mp4/mov, 2-15s each, total ≤15s)
- `reference_audios` (list[str]): 0-3 reference audio URLs or local file paths (mp3/wav, 2-15s each, total ≤15s)

**Optional per item - Video Output Parameters:**

- `ratio` (str): Aspect ratio. Options: "16:9" (default), "9:16", "4:3", "3:4", "1:1", "2:1", "21:9", "adaptive"
- `duration` (int): Video length in seconds. Range: 2-15s depending on model
...
Prefer executing the script by absolute path. Do not combine changing directories and running the Python script into a compound shell command; if the execution tool supports setting cwd, use the tool parameter for cwd.

```bash
# Text-to-Video
python3 {baseDir}/scripts/video_generate.py -p "小猫骑着滑板穿过公园" -n cat_park -r 16:9 -d 5 --resolution 720p

# First Frame Guidance
python3 {baseDir}/scripts/video_generate.py -p "小猫跳起来" -n cat_jump -f "https://example.com/cat.png" -r adaptive -d 5

# First Frame Guidance with local file
python3 {baseDir}/scripts/video_generate.py -p "小猫跳起来" -n cat_jump -f ./cat.png -r adaptive -d 5

# First + Last Frame Guidance
python3 {baseDir}/scripts/video_generate.py -p "平滑过渡动画" -n transition \
    -f "https://example.com/start.png" \
    -l "https://example.com/end.png" \
    -d 6

# Reference Images (style/content guidance)
python3 {baseDir}/scripts/video_generate.py -p "[图1]戴着眼镜的男生和[图2]柯基小狗坐在草坪上" -n styled \
    --ref-images "https://example.com/boy.png" "https://example.com/dog.png" \
    -r 16:9 -d 5

# Multimodal Reference (video + audio)
python3 {baseDir}/scripts/video_generate.py -p "将视频中的人物换成[图1]中的男孩" -n multimodal \
...
python3 {baseDir}/scripts/video_generate.py -p "女孩抱着狐狸，可以听到风声和树叶沙沙声" -n with_audio \
...
# Query task status
python3 {baseDir}/scripts/video_generate.py -q "cgt-20260222165751-wsnw8"

# Use specific model
python3 {baseDir}/scripts/video_generate.py -p "A futuristic city" -m doubao-seedance-1-5-pro-251215

# No watermark
python3 {baseDir}/scripts/video_generate.py -p "A beautiful landscape" --no-watermark
```
...
| `--duration` | `-d` | Video duration in seconds (2-15) |
| `--resolution` | | Video resolution: 480p, 720p, 1080p |
| `--first-frame` | `-f` | First frame image URL or local file path |
| `--last-frame` | `-l` | Last frame image URL or local file path |
| `--ref-images` | | Reference image URLs or local paths (space-separated, 1-4 images) |
| `--ref-videos` | | Reference video URLs or local paths (space-separated, 0-3 videos) |
| `--ref-audios` | | Reference audio URLs or local paths (space-separated, 0-3 audios) |
...
By default, the script uses `doubao-seedance-2-0-260128` from `{baseDir}/scripts/video_generate.py`.

If you encounter a model-related error (like `ModelNotOpen`) with the default model, downgrade in this order:
...
- All image/video/audio parameters accept both URLs and local file paths. Local files are automatically converted to base64 data URIs.
- Local file security: system paths (`/etc/`, `/proc/`, etc.), sensitive directories (`~/.ssh/`, `~/.aws/`, etc.), and non-media extensions are blocked. Max file size: 50MB (warning at 20MB).
