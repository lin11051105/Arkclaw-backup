...
def _validate_local_image_file(file_path: str) -> None:
    ...
    if ext not in _IMAGE_MIME_MAP:
        raise ValueError(
            f"File extension '{ext}' is not allowed for image input. "
            f"Allowed: {sorted(_IMAGE_MIME_MAP.keys())}"
        )
...
def _local_file_to_data_uri(file_path: str) -> str:
    _validate_local_image_file(file_path)
    ext = os.path.splitext(file_path)[1].lower()
    mime = _IMAGE_MIME_MAP.get(ext) or mimetypes.guess_type(file_path)[0]
    if not mime:
...
def _resolve_image_url(value: str) -> str:
    if _is_local_file(value):
        ...
        return _local_file_to_data_uri(value)
    return value


def _resolve_image_urls(values: list) -> list:
    return [_resolve_image_url(v) for v in values]
...
async def seedream_generate(
    ...
    for task in tasks:
        resolved = dict(task)
        if "image" in resolved and resolved["image"] is not None:
            img = resolved["image"]
            if isinstance(img, list):
                resolved["image"] = _resolve_image_urls(img)
            else:
                resolved["image"] = _resolve_image_url(img)
        resolved_tasks.append(resolved)

    coroutines = [
        handle_single_task(idx, item, model_name, version, timeout) 
        for idx, item in enumerate(resolved_tasks)
    ]
