...
DEFAULT_SKILL_CALLER = "arkclaw-business"
DEFAULT_SKILL_REGION = "cn-beijing"
...
def _resolve_skill_caller(skills_api_url: str) -> str:
    formatted = skills_api_url.strip()
    if not formatted:
        return DEFAULT_SKILL_CALLER

    try:
        path_segments = [segment for segment in urlparse(formatted).path.split("/") if segment]
    except Exception:
        return DEFAULT_SKILL_CALLER

    try:
        caller_index = path_segments.index("c")
    except ValueError:
        return DEFAULT_SKILL_CALLER

    if caller_index + 1 < len(path_segments):
        caller = path_segments[caller_index + 1]
        if caller:
            return caller
    return DEFAULT_SKILL_CALLER


def _resolve_skill_region(api_base: str) -> str:
    formatted = api_base.strip()
    if not formatted:
        return DEFAULT_SKILL_REGION

    try:
        hostname = urlparse(formatted).hostname or ""
    except Exception:
        return DEFAULT_SKILL_REGION

    gateway_label = next(
        (segment for segment in hostname.split(".") if segment.startswith("apigateway-")),
        "",
    )
    if not gateway_label:
        return DEFAULT_SKILL_REGION

    region = gateway_label[len("apigateway-") :]
    if region.endswith("-inner"):
        region = region[: -len("-inner")]
    return region or DEFAULT_SKILL_REGION


def _build_skill_headers(
    api_host: str,
    skills_api_url: str,
    operator_union_id: Optional[str],
    operator_user_email: Optional[str],
) -> Dict[str, str]:
    headers = {
        "X-Skill-Region": _resolve_skill_region(api_host),
        "X-Skill-Caller": _resolve_skill_caller(skills_api_url),
    }
    if operator_union_id:
        headers["X-Skill-OperatorUserId"] = operator_union_id
    if operator_user_email:
        headers["X-Skill-OperatorUser"] = operator_user_email
    return headers


def _read_env() -> Tuple[str, str, Optional[str], Dict[str, str]]:
    ...
    operator_union_id = os.getenv("OPERATOR_UNION_ID", "").strip()
    operator_user_email = os.getenv("OPERATOR_USER_EMAIL", "").strip()
    if not api_host or not api_key:
        raise SystemExit("缺少必要环境变量: ARK_SKILL_API_BASE, ARK_SKILL_API_KEY")
    sid = skill_space_id.strip() if skill_space_id else None
    if not sid:
        sid = None
    normalized_api_host = _normalize_api_base(api_host)
    return (
        normalized_api_host,
        api_key.strip(),
        sid,
        _build_skill_headers(
            normalized_api_host,
            skills_api_url,
            operator_union_id,
            operator_user_email,
...
def main():
    ...
    api_host, api_key, space_id, request_headers = _read_env()
    ...
    version_id, _ = extract_version_info(skill)
    zip_bytes, _ = download_skill(
        api_host,
        api_key,
        skill_id,
        version_id,
